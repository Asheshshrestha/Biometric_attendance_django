...
def process(request,card):
    card=card
    
    #card = request.GET.get('card_id', 'No Record Found')
...
def attend(user):
    if user.name == None:
        statu = 'Please save the profile first'
        return statu
    logs = Log.objects.all()
    for log in logs:
        if log.card_id == int(user.card_id):
            if str(log.date) == str(datetime.datetime.now())[:10]:
                if log.time_out == None:
                    ...
                    statu = 'you have already checkout the class'
                    return statu
...
def card(request):
    ...
    if request.method == 'POST':
        if request.POST.get("sel"):
            ids = request.POST.get('idsearch', 'No Id record Found')
            for user in users:
                if user.id == int(ids):
...
def give_command(request,data):
    mode = Mode_Type.objects.get(id_choice=1)
    data_input=data
    if data_input==1:
        mode.choose=1
        mode.save()
    ...
    if data_input==3:
        ...
        return HttpResponse("you are fucked!")

def get_command(request):
    mode = Mode_Type.objects.get(id_choice=1)
    print(mode.choose)
    return HttpResponse(mode.choose)
